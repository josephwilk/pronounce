  ...
  describe SyllableRules do
    ...
    describe '.evaluate' do
      ...
      it 'calls the Sonority Sequencing Principle last' do
        ...
        SyllableRules['Sonority Sequencing Principle'].should_receive :evaluate do
          final_rule_called = true
        end
        SyllableRules[:en]['stressed syllables cannot be light'].should_receive :evaluate do
          raise if final_rule_called
        end
        SyllableRules[:en]['/ng/ cannot start a syllable'].should_receive :evaluate do
          raise if final_rule_called
        end
      ...
      it 'returns the first boolean value returned by a rule' do
        SyllableRules[:en]['stressed syllables cannot be light'].should_receive :evaluate
        SyllableRules[:en]['/ng/ cannot start a syllable'].should_receive(:evaluate).and_return true
        SyllableRules['Sonority Sequencing Principle'].should_not_receive :evaluate
